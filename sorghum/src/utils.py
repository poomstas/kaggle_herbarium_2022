...
# %%
def balance_val_split(dataset, target_variable_name, val_size=0.2, random_state=None):
...
def get_stratified_sampler_for_subset(subset, target_variable_name):
    ...
    assert isinstance(superset, pd.DataFrame)
...
def get_dataloaders_train_val(csv_fullpath, target_variable_name, transforms, target_size, batch_size, num_workers,
    ...
    sampler_train, _ = get_stratified_sampler_for_subset(dataset_train, target_variable_name)
    sampler_val, _   = get_stratified_sampler_for_subset(dataset_val, target_variable_name)
